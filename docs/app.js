...
function initializeMap() {
  ...
  map = L.map("map").setView([35.6812, 139.7671], 3);
    ...
    if (placesContainer) {
      placesContainer.innerHTML = `
        <div class="empty-state">
          ...
          <p>Please check that places.json is valid and available.</p>
        </div>
      `;
    }
  }
...
async function prepareAllPlaces() {
  ...
  for (const place of places) {
    try {
      const prepared = await preparePlace(place);
      preparedPlaces.push(prepared);
    } catch (error) {
      console.error(`Failed to prepare ${place.name}:`, error);
      preparedPlaces.push({
        ...place,
        country: null,
    ...
    // Nominatim's public service should not be hit with many requests simultaneously.
    await delay(1100);
  }
...
async function preparePlace(place) {
  const location = await getPlaceLocation(place);
  const country = location.country;
  const description = await getWikipediaDescription(place, country);
  return {
    ...place,
    country: country,
...
async function getPlaceLocation(place) {
  const cacheKey = `location:${place.name}`;
  ...
  try {
    ...
    if (!Array.isArray(results) || results.length === 0) {
      const result = { address: ADDRESS_PLACEHOLDER, lat: null, lng: null, country: null };
    ...
    const location = {
      address: result.display_name || ADDRESS_PLACEHOLDER,
      lat: result.lat ? parseFloat(result.lat) : null,
      lng: result.lon ? parseFloat(result.lon) : null,
      country: countryCode
    };

    placeCache[cacheKey] = location;
    return location;
  } catch (error) {
    console.error(`Nominatim error for ${place.name}:`, error);
    const fallback = { address: ADDRESS_PLACEHOLDER, lat: null, lng: null, country: null };
    placeCache[cacheKey] = fallback;
    return fallback;
  }
}
...
// ============================================================
async function getWikipediaDescription(place, countryCode) {
  const cacheKey = `description:${place.name}:${countryCode || ""}`;
  if (placeCache[cacheKey]) {
    return placeCache[cacheKey];
  }
  ...
  try {
    ...
    const searchText = countryName ? `${place.name} ${countryName}` : place.name;
    ...
    if (!searchData.pages || searchData.pages.length === 0) {
      placeCache[cacheKey] = DESCRIPTION_PLACEHOLDER;
      return DESCRIPTION_PLACEHOLDER;
    }

    const placeName = place.name.toLowerCase();
    let matchingPage = searchData.pages.find(page =>
      page.title.toLowerCase().includes(placeName)
    );

    if (!matchingPage) {
      matchingPage = searchData.pages[0];
    }
    ...
    placeCache[cacheKey] = description;
    return description;
  } catch (error) {
    console.error(`Wikipedia error for ${place.name}:`, error);
    placeCache[cacheKey] = DESCRIPTION_PLACEHOLDER;
    return DESCRIPTION_PLACEHOLDER;
  }
}
...
function initializeApplication() {
  ...
  if (urlCountry && countryExists(urlCountry)) {
    setCountry(urlCountry);
    populateRegionForCountry(urlCountry);
  } else {
    const firstPlaceWithCountry = places.find(place => place.country && countryExists(place.country));
    if (firstPlaceWithCountry) {
      setCountry(firstPlaceWithCountry.country);
      populateRegionForCountry(firstPlaceWithCountry.country);
    } else {
      setCountry("JP");
...
function setupRegionSelector() {
  ...
  regionSelect.addEventListener("change", function () {
    ...
    const firstCountry = Object.keys(countryRegions[region].countries)[0];
    if (firstCountry) {
...
function populateCountries(regionKey) {
  ...
  if (!regionKey || !countryRegions[regionKey]) {
    return;
  }

  const countries = countryRegions[regionKey].countries;
...
function countryExists(countryCode) {
  ...
  for (const regionKey in countryRegions) {
    if (countryRegions[regionKey].countries[countryCode]) {
      return true;
    }
  }
  return false;
...
function getCountryInfo(countryCode) {
  ...
  for (const regionKey in countryRegions) {
    ...
    if (region.countries[countryCode]) {
      return { ...region.countries[countryCode], region: regionKey };
    }
  }
  return null;
}
...
function getFilteredPlaces() {
  return places.filter(place => {
    ...
    return Array.isArray(place.categories) && place.categories.includes(currentCategory);
  });
}
...
function render() {
  ...
  renderCount(filteredPlaces.length);
...
function renderCount(count) {
  ...
  resultCount.textContent = `${count} recommendation${count === 1 ? "" : "s"}`;
}
...
function getURLType(url) {
  ...
  try {
    ...
    // Meetup can appear as meetup.com, www.meetup.com, etc.
...
function getExternalLinkHTML(place) {
  const urlType = getURLType(place.url);
  if (!urlType) {
    return "";
  }
  if (urlType === "meetup") {
...
function renderCards(filteredPlaces) {
  ...
  if (filteredPlaces.length === 0) {
    placesContainer.innerHTML = `
      <div class="empty-state">
        ...
        <p>There are currently no places matching this country and category.</p>
      </div>
    `;
    return;
  }
  ...
  filteredPlaces.forEach(place => {
    ...
    const categoriesHTML = Array.isArray(place.categories)
      ? place.categories
          .map(
            category => `
    ...
    const externalLinkHTML = getExternalLinkHTML(place);

    card.innerHTML = `
      <h3 class="place-name">${escapeHTML(place.name)}</h3>
      <div class="place-categories">${categoriesHTML}</div>
      <p class="place-description">${escapeHTML(place.description || DESCRIPTION_PLACEHOLDER)}</p>
      <div class="place-address">
        <span class="address-icon">📍</span>
        <span>${escapeHTML(place.address || ADDRESS_PLACEHOLDER)}</span>
      </div>
      <div class="place-actions">
        ${externalLinkHTML}
        <button class="copy-button" type="button">Copy address</button>
      </div>
    `;
    ...
    if (copyButton) {
      copyButton.addEventListener("click", function () {
        copyAddress(place.address || ADDRESS_PLACEHOLDER, this);
      });
    }
...
function renderMarkers(filteredPlaces) {
  ...
  const placesWithCoordinates = filteredPlaces.filter(
    place =>
  ...
  if (markers.length === 1) {
    map.setView([placesWithCoordinates[0].lat, placesWithCoordinates[0].lng], 14);
  } else {
